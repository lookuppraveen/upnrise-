// Trainee · Feedbacks (received)
//
// All feedback rows where the trainee is the recipient. AI summaries land
// here automatically after each scored roleplay session; human notes from
// managers will land here once that flow ships. Clicking a session-linked
// row jumps to the results page so the trainee can see their transcript.

import Link from "next/link";
import { getSessionUser } from "@/lib/auth/session";
import { prisma } from "@/lib/db/client";
import { Icon, type IconName } from "@/components/ui/Icon";
import { cn } from "@/lib/cn";

type Tab = "all" | "ai" | "human" | "needs_work";

const TABS: Array<{ key: Tab; label: string }> = [
  { key: "all", label: "All" },
  { key: "ai", label: "AI coach" },
  { key: "human", label: "Manager notes" },
  { key: "needs_work", label: "Needs work" },
];

export default async function TraineeFeedbacksPage({
  searchParams,
}: {
  searchParams: Promise<{ tab?: string }>;
}) {
  const sp = await searchParams;
  const tab = (TABS.find((t) => t.key === sp.tab)?.key ?? "all") as Tab;

  const user = (await getSessionUser())!;

  const all = await prisma.feedback.findMany({
    where: { recipientId: user.id },
    orderBy: { createdAt: "desc" },
    take: 200,
    include: {
      training: { select: { id: true, title: true } },
      session: {
        select: {
          id: true,
          score: true,
          module: { select: { id: true, training: { select: { id: true } } } },
        },
      },
    },
  });

  const counts: Record<Tab, number> = {
    all: all.length,
    ai: all.filter((f) => f.kind === "ai").length,
    human: all.filter((f) => f.kind === "human").length,
    needs_work: all.filter((f) => f.sentiment === "negative").length,
  };

  const filtered = all.filter((f) => {
    if (tab === "all") return true;
    if (tab === "ai") return f.kind === "ai";
    if (tab === "human") return f.kind === "human";
    return f.sentiment === "negative";
  });

  const lastWeek = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
  const last7d = all.filter((f) => +f.createdAt >= +lastWeek).length;
  const linkedSessions = all.filter((f) => f.session != null);
  const avgScore =
    linkedSessions.length > 0
      ? Math.round(
          linkedSessions.reduce(
            (acc, f) => acc + (f.session?.score ?? 0),
            0,
          ) / linkedSessions.length,
        )
      : null;

  return (
    <div className="px-7 pt-6 pb-20 max-w-[1100px] space-y-5">
      <Header total={all.length} />
      <KpiStrip
        last7d={last7d}
        aiCount={counts.ai}
        humanCount={counts.human}
        needsWork={counts.needs_work}
        avgScore={avgScore}
      />

      <PillTabs current={tab} counts={counts} />

      {filtered.length === 0 ? (
        <EmptyState tab={tab} />
      ) : (
        <div className="space-y-3">
          {filtered.map((f) => (
            <FeedbackCard
              key={f.id}
              row={{
                id: f.id,
                kind: f.kind,
                category: f.category,
                sentiment: f.sentiment,
                body: f.body,
                source: f.source,
                createdAt: f.createdAt,
                training: f.training,
                session: f.session,
              }}
            />
          ))}
        </div>
      )}
    </div>
  );
}

// ─────────────── Header ───────────────

function Header({ total }: { total: number }) {
  return (
    <header className="space-y-2">
      <h1 className="font-display text-[36px] leading-[1.05] -tracking-[0.015em] text-accent">
        Feedbacks
      </h1>
      <p className="text-ink-2 text-[13.5px] max-w-[640px] leading-[1.5]">
        Coach notes and manager feedback addressed to you. Every roleplay
        you complete generates an AI summary; managers can add human
        notes once that flow ships.
        {total > 0 ? ` ${total} item${total === 1 ? "" : "s"} so far.` : ""}
      </p>
    </header>
  );
}

// ─────────────── KPI ───────────────

function KpiStrip({
  last7d,
  aiCount,
  humanCount,
  needsWork,
  avgScore,
}: {
  last7d: number;
  aiCount: number;
  humanCount: number;
  needsWork: number;
  avgScore: number | null;
}) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-[14px]">
      <KpiTile
        label="Last 7 days"
        value={String(last7d)}
        sub={
          last7d === 0
            ? "nothing this week"
            : `${last7d} note${last7d === 1 ? "" : "s"} landed`
        }
        icon="message"
        tone="accent"
      />
      <KpiTile
        label="Linked avg score"
        value={avgScore != null ? `${avgScore}` : "—"}
        sub={
          avgScore == null
            ? "no scored sessions"
            : avgScore >= 75
              ? "strong band"
              : avgScore >= 60
                ? "room to grow"
                : "below target"
        }
        icon="chart"
        tone={
          avgScore == null || avgScore >= 75
            ? "good"
            : "warn"
        }
      />
      <KpiTile
        label="AI vs human"
        value={`${aiCount} / ${humanCount}`}
        sub={
          aiCount + humanCount === 0
            ? "nothing yet"
            : humanCount === 0
              ? "all AI-generated"
              : `${Math.round((aiCount / (aiCount + humanCount)) * 100)}% AI`
        }
        icon="ai-sparkle"
        tone="violet"
      />
      <KpiTile
        label="Needs work"
        value={String(needsWork)}
        sub={
          needsWork === 0
            ? "nothing flagged"
            : "things to focus on"
        }
        icon="alert"
        tone={needsWork > 0 ? "warn" : "good"}
      />
    </div>
  );
}

function KpiTile({
  label,
  value,
  sub,
  icon,
  tone,
}: {
  label: string;
  value: string;
  sub: string;
  icon: IconName;
  tone: "accent" | "good" | "warn" | "violet";
}) {
  const corner =
    tone === "accent"
      ? "bg-accent-pale text-accent-strong"
      : tone === "good"
        ? "bg-good-pale text-good"
        : tone === "warn"
          ? "bg-warn-pale text-warn"
          : "bg-[#ede9fe] text-[#6d4ad9]";
  return (
    <div className="relative bg-surface border border-border rounded-[12px] px-[18px] py-4 flex flex-col gap-1">
      <div className="text-[10.5px] font-bold uppercase tracking-[0.1em] text-ink-3">
        {label}
      </div>
      <div className="font-display text-[32px] leading-[1.05] -tracking-[0.01em] mt-1">
        {value}
      </div>
      <div className="text-[11.5px] text-ink-3 mt-[3px] truncate">{sub}</div>
      <div
        className={cn(
          "absolute top-[14px] right-[14px] w-[28px] h-[28px] rounded-[7px] grid place-items-center",
          corner,
        )}
      >
        <Icon name={icon} size={13} />
      </div>
    </div>
  );
}

// ─────────────── Pill tabs ───────────────

function PillTabs({
  current,
  counts,
}: {
  current: Tab;
  counts: Record<Tab, number>;
}) {
  return (
    <div className="flex items-center gap-2 flex-wrap">
      {TABS.map((t) => {
        const active = t.key === current;
        return (
          <Link
            key={t.key}
            href={
              t.key === "all" ? "/learn/feedbacks" : `/learn/feedbacks?tab=${t.key}`
            }
            className={cn(
              "inline-flex items-center gap-2 px-3 py-[6px] rounded-full text-[12.5px] font-semibold transition-colors",
              active
                ? "bg-ink text-white"
                : "bg-surface border border-border text-ink-2 hover:text-ink hover:border-border-strong",
            )}
          >
            {t.label}
            <span
              className={cn(
                "text-[10.5px] font-mono px-1.5 py-[1px] rounded-full",
                active
                  ? "bg-white/15 text-white/80"
                  : t.key === "needs_work" && counts.needs_work > 0
                    ? "bg-bad-pale text-bad"
                    : "bg-surface-2 text-ink-3",
              )}
            >
              {counts[t.key]}
            </span>
          </Link>
        );
      })}
    </div>
  );
}

// ─────────────── Feedback card ───────────────

type Row = {
  id: string;
  kind: "ai" | "human";
  category:
    | "roleplay"
    | "quiz"
    | "pronunciation"
    | "platform"
    | "content"
    | "other";
  sentiment: "positive" | "neutral" | "negative" | null;
  body: string;
  source: string | null;
  createdAt: Date;
  training: { id: string; title: string } | null;
  session: {
    id: string;
    score: number | null;
    module: { id: string; training: { id: string } } | null;
  } | null;
};

function FeedbackCard({ row }: { row: Row }) {
  const score = row.session?.score ?? null;
  const scoreTone =
    score == null
      ? "text-ink-3"
      : score >= 75
        ? "text-good"
        : score >= 60
          ? "text-warn"
          : "text-bad";
  const resultsHref =
    row.session && row.session.module
      ? `/learn/trainings/${row.session.module.training.id}/modules/${row.session.module.id}/results/${row.session.id}`
      : null;

  return (
    <article className="bg-surface border border-border rounded-[12px] p-5 flex items-start gap-4">
      <div className="shrink-0 w-[64px] text-center">
        {score != null ? (
          <>
            <div
              className={cn(
                "font-display text-[28px] leading-none -tracking-[0.01em]",
                scoreTone,
              )}
            >
              {score}
            </div>
            <div className="text-[10px] font-bold uppercase tracking-[0.1em] text-ink-3 mt-1">
              Score
            </div>
          </>
        ) : (
          <div className="font-mono text-[11px] text-ink-3 pt-3">—</div>
        )}
      </div>

      <div className="flex-1 min-w-0 space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          {row.training ? (
            <Link
              href={`/learn/trainings/${row.training.id}`}
              className="text-[12.5px] text-accent hover:text-accent-strong hover:underline truncate"
            >
              {row.training.title}
            </Link>
          ) : (
            <span className="text-[12.5px] text-ink-3">Platform note</span>
          )}
          <span className="text-[11px] font-mono text-ink-3 ml-auto shrink-0">
            {fmtRelative(row.createdAt)}
          </span>
        </div>

        <p className="text-[13px] text-ink leading-[1.6] whitespace-pre-wrap">
          {row.body}
        </p>

        <div className="flex items-center gap-2 flex-wrap pt-1">
          <KindChip kind={row.kind} />
          <CategoryChip category={row.category} />
          {row.sentiment ? <SentimentPill sentiment={row.sentiment} /> : null}
          {row.source ? (
            <span className="text-[10.5px] font-mono text-ink-3">
              {row.source}
            </span>
          ) : null}
          {resultsHref ? (
            <Link
              href={resultsHref}
              className="ml-auto inline-flex items-center gap-1 text-[11.5px] font-semibold text-accent hover:text-accent-strong"
            >
              See transcript
              <Icon name="chevron-right" size={11} />
            </Link>
          ) : null}
        </div>
      </div>
    </article>
  );
}

function KindChip({ kind }: { kind: "ai" | "human" }) {
  if (kind === "ai") {
    return (
      <span
        className="inline-flex items-center gap-1 text-[10.5px] font-bold uppercase tracking-[0.08em] px-2 py-1 rounded-[6px] text-[#6d4ad9]"
        style={{ background: "#ede9fe" }}
      >
        <Icon name="ai-sparkle" size={10} />
        AI Coach
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 text-[10.5px] font-bold uppercase tracking-[0.08em] px-2 py-1 rounded-[6px] bg-surface-2 text-ink-2">
      <Icon name="users" size={10} />
      Manager
    </span>
  );
}

function CategoryChip({ category }: { category: Row["category"] }) {
  const label = {
    roleplay: "Roleplay",
    quiz: "Quiz",
    pronunciation: "Pronunciation",
    platform: "Platform",
    content: "Content",
    other: "Other",
  }[category];
  const cls = {
    roleplay: "bg-accent-pale text-accent-strong",
    quiz: "bg-[#fef0e7] text-[#c2410c]",
    pronunciation: "bg-[#ede9fe] text-[#6d4ad9]",
    platform: "bg-surface-2 text-ink-2",
    content: "bg-good-pale text-good",
    other: "bg-surface-2 text-ink-3",
  }[category];
  return (
    <span
      className={cn(
        "inline-flex items-center text-[10.5px] font-bold uppercase tracking-[0.08em] px-2 py-1 rounded-[6px]",
        cls,
      )}
    >
      {label}
    </span>
  );
}

function SentimentPill({
  sentiment,
}: {
  sentiment: "positive" | "neutral" | "negative";
}) {
  const cfg = {
    positive: { label: "On track", cls: "bg-good-pale text-good", dot: "bg-good" },
    neutral: { label: "Steady", cls: "bg-surface-2 text-ink-2", dot: "bg-ink-3" },
    negative: { label: "Focus", cls: "bg-bad-pale text-bad", dot: "bg-bad" },
  } as const;
  const m = cfg[sentiment];
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 text-[10.5px] font-bold uppercase tracking-[0.08em] px-2 py-1 rounded-[6px]",
        m.cls,
      )}
    >
      <span className={cn("w-1.5 h-1.5 rounded-full", m.dot)} />
      {m.label}
    </span>
  );
}

// ─────────────── Empty ───────────────

function EmptyState({ tab }: { tab: Tab }) {
  const copy =
    tab === "all"
      ? "No feedback yet. Once you finish a scored roleplay, AI Coach notes show up here."
      : tab === "ai"
        ? "No AI Coach notes yet. They're written automatically after each scored roleplay."
        : tab === "human"
          ? "No manager notes yet. Once that flow ships, your manager's feedback will land here."
          : "Nothing flagged as Focus. Keep at it.";
  return (
    <div className="bg-surface border border-border rounded-[12px] p-10 text-center">
      <div className="w-12 h-12 rounded-[10px] bg-surface-2 text-ink-3 grid place-items-center mx-auto">
        <Icon name="message" size={18} />
      </div>
      <h3 className="font-display text-[18px] mt-3 text-ink">
        Nothing here yet
      </h3>
      <p className="text-[13px] text-ink-2 mt-1 max-w-[420px] mx-auto leading-[1.55]">
        {copy}
      </p>
    </div>
  );
}

// ─────────────── Helpers ───────────────

function fmtRelative(d: Date): string {
  const mins = Math.round((Date.now() - d.getTime()) / 60000);
  if (mins < 1) return "just now";
  if (mins < 60) return `${mins}m ago`;
  const hours = Math.round(mins / 60);
  if (hours < 24) return `${hours}h ago`;
  const days = Math.round(hours / 24);
  if (days < 7) return `${days}d ago`;
  return d.toLocaleDateString("en-US", { month: "short", day: "numeric" });
}
